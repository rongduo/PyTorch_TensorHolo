...
class HologramAgent(BaseAgent):
    ...
    def recon_focal_stack(self, input_depth, out_amp, out_phs, target_amp, target_phs, recon_dist, beta=0.35):        
        ...
        for d in range(recon_dist.shape[0]): # traverse for all recon distances
            ...
            # Add Oct. 22, 2023, add torch.abs() for the distance difference and normalize the weight map
    ...
    def forward(self, data):
        ...
        out_amp = (out_amp + 1.0) / 2.0 #np.sqrt(2) # from [-1, 1] to [0, sqrt(2)]
        if not self.config.is_train:
            out_amp = self.scale(out_amp, target_amp)
        ...
        for m in range(input_img.size(0)): # enumeate on the batch dimension
            ...
            # Modified on Oct. 22, 2023. Since I found that in Liang's paper, they didn't calculate the intensity
            cur_pred_intensity = cur_pred_stack
            cur_tgt_intensity = cur_tgt_stack
