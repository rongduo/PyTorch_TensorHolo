...
class ImageLoader:
    ...
    def load_image(self, filenum, *augmentation_states):
        ...
        #print('depth_name is ', depth_name)
        ...
        # if in 'test' or 'validate' mode, uniformly sample 30 planes within the focal stack
        # if in 'train' mode, sample depth planes within the depth bins according to content distribution
        if not 'train_384' in self.data_path:
            recon_depth_array = np.linspace(0, 1, 30) * self.volume_width + self.near_plane
        else:
            ...
            concate_bins_count = np.sort(concate_bins_count, axis=1)
...
def get_image_filenames(dir):
    ...
    if 'train' in dir: 
        ...
        for line in reader:
            cur_path = line.strip()
            cur_path = os.path.join(dir, '/'.join(cur_path.split('/')[-2:]))
