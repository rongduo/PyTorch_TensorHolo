...
from algorithms import gerchberg_saxton, stochastic_gradient_descent, double_phase_amplitude_coding, stochastic_gradient_descent_joint
...
class SGD_RGB(nn.Module):
    ...
    final_phase: optimized phase-only representation at SLM plane, same dimensions
    """
    def __init__(self, dist_list, wave_list, feature_size, num_iters, roi_res, phase_path=None, prop_model='ASM',
                 propagator=None, loss=nn.MSELoss(), lr=0.01, lr_s=0.003, s0=1.0, writer=None,
                 device=torch.device('cuda')):
        super(SGD_RGB, self).__init__()
    ...
    def forward(self, target_amp, init_phase=None):
        ...
        if self.precomputed_H is None and self.prop_model == 'ASM':
            for c in range(3):
                precomputed_H = self.prop(torch.empty(*init_phase.shape, 2), self.feature_size,
                                            self.wave_list[c], self.dist_list[c], return_H=True)
                precomputed_H = precomputed_H.to(self.dev).detach()
                precomputed_H.requires_grad = False
                preH_list.append(precomputed_H)
        
        # Run algorithm
        final_phase = stochastic_gradient_descent_joint(init_phase, target_amp, self.num_iters, 
                                                   self.feature_size,self.wave_list,self.dist_list,
                                                  roi_res=self.roi_res, phase_path=self.phase_path,
                                                  prop_model=self.prop_model, propagator=self.prop,
                                                  loss=self.loss, lr=self.lr, lr_s=self.lr_s, s0=self.init_scale,
                                                  writer=self.writer,
