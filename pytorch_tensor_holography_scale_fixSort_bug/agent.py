...
class HologramAgent(BaseAgent):
    ...
    def recon_focal_stack(self, input_depth, out_amp, out_phs, target_amp, target_phs, recon_dist, beta=0.35):
        ...
        depth_transform = input_depth * self.config.depth_scale + self.config.depth_base 
        stack_amp  = torch.stack([out_amp, target_amp], dim=0)
        stack_phase = torch.stack([out_phs, target_phs], dim=0)
        for d in range(recon_dist.shape[0]): # traverse for all recon distances
            cur_dist = recon_dist[d].item() * 1e-3
            #print('cur_dist is ', cur_dist)
            recon_amps = recon_sgdHolo_specific(stack_amp, stack_phase, self.config.wavelength_list,
                                            cur_dist, feature_size = self.config.feature_size)
            predict_recons.append(recon_amps[0,...]) # first is the recon for predicted hologram
            target_recons.append(recon_amps[1,...]) # second is the recon for target hologram

            # calculate the attention map at current reconstruction distance 
            cur_att_map = torch.exp(beta * (self.config.depth_scale - (cur_dist * 1e3 - input_depth_transform)))
            attention_maps.append(cur_att_map)
        
        pred_recon_stack = torch.stack(predict_recons, dim=0)
        tgt_recon_stack = torch.stack(target_recons, dim=0)
        att_map_stack = torch.stack(attention_maps, dim=0)

        #print(pred_recon_stack.shape, tgt_recon_stack.shape, att_map_stack.shape)
        
        return pred_recon_stack, tgt_recon_stack, att_map_stack
    ...
    def forward(self, data):
        ...
        out_amp = (out_amp + 1.0) / 2.0 # from [-1, 1] to [0, 1]
        ...
        correct_diff_phs = delta_phs - torch.mean(delta_phs, dim=(2,3), keepdim=True)  # the phase term in Formula (3) of the paper
        
        correct_cpx_real, correct_cpx_imag = utils.polar_to_rect(target_amp, correct_diff_phs)
        correct_cpx = torch.stack([correct_cpx_real, correct_cpx_imag], dim=-1)
        ...
        # first calculate the norm for each element and then calculate mean on all elements
        data_term = torch.mean(torch.norm(torch.abs(out_field - correct_cpx) + 1e-6, dim=-1), dim=(0,1,2,3))   # formula (3) 
        ...
        for m in range(input_img.size(0)): # enumeate on the batch dimension
            cur_pred_stack, cur_tgt_stack, cur_att_stack = self.recon_focal_stack(input_depth[m,...], out_amp[m,...],
                                out_phase[m,...], target_amp[m,...], target_phase[m,...], recon_dists[m])
    ...
    def visualize_batch(self, data, mode, outputs=None):
        ...
        out_amp = out_amp[-1,...].cpu()
        out_phs = out_phs[-1,...].cpu() # from bgr to rgb
        ...
        for k in range(3):
            pred_recon_amp = select_stacks_pred[k, ...].cpu()
            tgt_recon_amp = select_stacks_tgt[k, ...].cpu()            
        ...
        phase_image = self.bgr2rgb(phase_image)
        amp_image = torch.cat([out_amp, v_line, target_amp], dim=2)
        amp_image = self.bgr2rgb(amp_image)
        input_img = self.bgr2rgb(input_img)
